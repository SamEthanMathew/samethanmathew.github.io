...
export interface OptionWheelProps {
  ...
  /** Fired whenever the wheel settles on a new option (scroll, drag, click). */
  onChange?: (index: number, item: string) => void;
  /** Fired only on an explicit press/Enter on an option — use this to navigate. */
  ...
  inset?: number;
  loop?: boolean;
  /** When not looping, let the page scroll past once the wheel hits its first/last option. */
  releaseAtEnds?: boolean;
  draggable?: boolean;
  soundUrl?: string;
  soundVolume?: number;
  className?: string;
...
const OptionWheel = ({
  ...
  useEffect(() => {
    ...
    const onWheel = (e: globalThis.WheelEvent) => {
      const cfg = cfgRef.current;
      // Once the wheel is parked at its first/last option, hand further scroll
      // in that direction back to the page (so it can reach the hero / footer).
      if (cfg.releaseAtEnds && !cfg.loop) {
        const atTop = targetRef.current <= 0.0001;
        const atBottom = targetRef.current >= cfg.count - 1 - 0.0001;
  ...
  const handleItemClick = useCallback(
    (index: number) => {
      ...
      // Explicit press → activate (navigate). Scrolling never reaches here.
  ...
  useEffect(
    () => () => {
      // Null the handle after cancelling so a StrictMode remount can restart
      // the loop — otherwise startLoop's `rafRef.current != null` guard blocks
      // it forever and every option stays stacked at the center.
